...
const getChangeHistoryLabel = (reservation: Reservation) => {
  if (!reservation.changeAppliedAt) return '—';
  return '変更処理済み';
};

export function AdminReservationsEnhancedPage() {
  ...
  const summary = useMemo(() => {
    ...
    const todayCount = reservations.filter((reservation) => new Date(reservation.reservationDatetime).toLocaleDateString('sv-SE') === todayDate).length;
    return {
  ...
  const commitReservationStatusChange = async (reservationId: string, nextStatus: ReservationStatus) => {
    ...
    try {
      ...
      setReservations((prev) => prev.map((reservation) => (reservation.id === updatedReservation.id ? updatedReservation : reservation)));
    } catch {
  ...
  const handleExportCsv = () => {
    ...
    const headers = [
      '予約ID', '店舗名', '予約者名', '電話番号', 'メールアドレス', '予約日時', '人数', 'ステータス',
      'キャンセル理由', 'キャンセル依頼', 'キャンセル依頼理由', '予約変更依頼', '希望日時', '希望人数', '変更依頼内容',
      '変更処理日時', '変更前日時', '変更前人数', '変更後日時', '変更後人数', '変更時のご要望', '予約変更完了メール',
      '管理メモ', '備考', '申込日時',
    ];
    ...
    const rows = filteredReservations.map((reservation) => [
      ...
      reservation.changeBeforeDatetime ?? '',
      reservation.changeBeforePartySize ?? '',
      reservation.changeAfterDatetime ?? '',
      reservation.changeAfterPartySize ?? '',
      reservation.changeAppliedNote ?? '',
      reservation.changeCompletionEmailSentAt ?? '',
  ...
  return (
    <section className="card detail-wrapper">
      ...
      <section className="reservation-filter-grid" aria-label="予約の検索と絞り込み">
        <div>
          <label htmlFor="reservation-search">店舗名 / 予約者名 / 電話番号 / メールアドレス / 変更履歴で検索</label>
          <input id="reservation-search" type="search" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="例: 山田 / 090 / 予定変更" />
        </div>
        <div>
          <label htmlFor="reservation-status-filter">ステータスで絞り込み</label>
          <select id="reservation-status-filter" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as (typeof STATUS_OPTIONS)[number])}>
            {STATUS_OPTIONS.map((status) => <option key={status} value={status}>{FILTER_LABEL[status]}</option>)}
          </select>
        </div>
        <div>
          ...
          <select id="reservation-date-filter" value={dateFilter} onChange={(event) => setDateFilter(event.target.value as DateFilter)}>
            {DATE_FILTER_OPTIONS.map((option) => <option key={option} value={option}>{DATE_FILTER_LABEL[option]}</option>)}
          </select>
        </div>
        <div>
          ...
          <select id="reservation-sort-option" value={sortOption} onChange={(event) => setSortOption(event.target.value as SortOption)}>
            {SORT_OPTIONS.map((option) => <option key={option} value={option}>{SORT_LABEL[option]}</option>)}
          </select>
        </div>
      </section>
              ...
              {filteredReservations.map((reservation) => {
                ...
                return (
                  <tr key={reservation.id} className={getReservationRowClassName(reservation)}>
                    ...
                    <td>{formatDateTime(reservation.reservationDatetime)}{isUrgentPendingReservation(reservation) ? <span className="reservation-urgent-badge">24時間以内の予約</span> : null}</td>
                    ...
                    <td>
                      <div className="reservation-status-actions">
                        ...
                        {reservation.cancelRequestedAt ? <span className="reservation-cancel-request-badge">キャンセル依頼あり</span> : null}
                        {reservation.changeRequestedAt ? <span className="reservation-cancel-request-badge">変更依頼あり</span> : null}
                    ...
                    <td>
                      {reservation.cancelRequestedAt ? (
                        <div className="reservation-memo-display"><strong>キャンセル依頼あり</strong><p>{reservation.cancelRequestReason?.trim() ? reservation.cancelRequestReason : '理由未入力'}</p></div>
                      ) : '—'}
                    </td>
                    <td>
                    ...
                    <td>
                      {reservation.changeAppliedAt ? (
                        <div className="reservation-memo-display">
                          <strong>{getChangeHistoryLabel(reservation)}</strong>
                          <p>処理日時: {formatDateTime(reservation.changeAppliedAt)}</p>
                          <p>変更前: {reservation.changeBeforeDatetime ? formatDateTime(reservation.changeBeforeDatetime) : '—'} / {reservation.changeBeforePartySize ? `${reservation.changeBeforePartySize}名` : '—'}</p>
                          <p>変更後: {reservation.changeAfterDatetime ? formatDateTime(reservation.changeAfterDatetime) : '—'} / {reservation.changeAfterPartySize ? `${reservation.changeAfterPartySize}名` : '—'}</p>
                    ...
                    <td>
                      {isEditingMemo ? (
                        <div className="reservation-memo-editor">
                          <textarea value={memoDraft} onChange={(event) => setMemoDraft(event.target.value)} rows={3} />
